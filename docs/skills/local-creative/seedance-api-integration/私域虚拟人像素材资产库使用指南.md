...
:::warning
调用素材资产（Assets）API 接口需使用 Access Key 鉴权，详情参考 [获取 API 访问密钥（AK/SK）](https://www.volcengine.com/docs/6257/64983?lang=zh)。
:::
...
* **素材库**[项目](https://www.volcengine.com/docs/82379/1359411?lang=zh#03ec4a65)**（Project）隔离说明**
...
:::tip
该字段仅用于使用 ListAssets 接口时模糊搜索素材，不会被带入模型推理。关于如何使用素材生成视频，请参考[使用虚拟人像](/docs/82379/2291680#2bf01416) 和[3. 提示词（content.text）中应该如何准确指代参考素材？](/docs/82379/2333565#15e21eb8)。
:::
...
3. 根据 Status 判断
   * Processing → 继续轮询
   * Active → 返回 URL（结束）状态为 `Active` 后，可使用该素材 Asset ID (URI格式) 进行视频生成，如何使用人像素材生成视频，详见[使用虚拟人像](/docs/82379/2291680#2bf01416)。
   * Failed → 返回错误（结束）
4. 返回结果并打印结果
...
调用示例请参考[3. 提示词（content.text）中应该如何准确指代参考素材？](/docs/82379/2333565#15e21eb8)
:::
示例代码：
```Python
import os
...
#### 3. 提示词（content.**text**）中应该如何准确指代参考素材？
